...
このリポジトリは「正しく動く完成形」です。商品をカートに入れて注文し、合計金額（税込）やクーポン割引を計算し、注文のキャンセルで在庫を戻す、という一連の流れがすべて動きます。

> フロントエンドは別リポジトリ（[training-frontend](https://github.com/sebayashi-tomoya/training-frontend)）にあります。サーバー単体でも Swagger UI から API を叩いて動作確認できます。
...
```bash
git clone https://github.com/sebayashi-tomoya/training-backend-csharp.git
...
フロント（[training-frontend](https://github.com/sebayashi-tomoya/training-frontend)）を別ディレクトリに clone し、`config.js` の API ベース URL をこのサーバー（`http://localhost:5000`）に合わせて起動してください。
...
```
src/
├── Controllers/      API の入口（ProductsController, OrdersController）
├── Services/         業務ロジック（OrderService, PricingService）
├── Repositories/     DB アクセス（ProductRepository, OrderRepository）
